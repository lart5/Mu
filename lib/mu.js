...
Mu.normalize = function Mu_normalize(context, name) {
  ...
  if (/\./.test(name)) {
    var stack = [context]
    name.split(/\./g).forEach(function(current) {
	  var last = stack[stack.length - 1];
	  if (!last) throw new Error("Cannot find " + name + " (broken tree at " + current + " - its parent is null)");
 ...
 *        is a true boolean.
 * @param {String} name The name of the value to take from the context.
 * @param {Boolean} negative Invert the value taken from the context. 
 * @param {Function} fn The callback.
 */
Mu.enumerable = function Mu_enumerable(context, name, negative, fn) {
  if (/\./.test(name)) {
    var stack = [context]
    name.split(/\./g).forEach(function(current) {
	  var last = stack[stack.length - 1];
