...
function compile(parsed) {
  ...
  //var func = eval(code);
  var func = Script.runInNewContext(code, {RenderEventEmitter:RenderEventEmitter, M:Mu, process:process});
    ...
    return tokens.map(function (token) {
      ...
      case 'start_enumerable':
        return 'Mu.enumerable(' +
          'context, "' + value.replace(/"/, '\\"') + '", false, function (context) {';
      
      case 'start_inverted_enumerable':
        return 'Mu.enumerable(' +
          'context, "' + value.replace(/"/, '\\"') + '", true, function (context) {';
